...
class ActorNet:
    def __init__(self):
        ...
        with self.g.as_default():
            ...
            self.loss = tf.reduce_sum(tf.pow(self.y-self.fc2,2))
    ...
    def evaluate_actor(self,x,x_,is_training=False):
        return self.sess.run(self.pool9_flat, feed_dict={self.x:x,self.x_:x_,self.is_training:is_training}) 
    ...
    def train_actor(self,x,x_,y,is_training=True):
        self.sess.run([self.train_step,self.loss,self.conv1_bn.train_mean,self.conv1_bn.train_var,self.conv2_bn.train_mean,self.conv2_bn.train_var,self.conv3_bn.train_mean,\
            self.conv3_bn.train_var,self.conv4_bn.train_mean, self.conv4_bn.train_var,self.conv5_bn.train_mean, self.conv5_bn.train_var,self.conv1_bn_.train_mean,\
            self.conv1_bn_.train_var,self.conv2_bn_.train_mean,self.conv2_bn_.train_var,self.conv3_bn_.train_mean,self.conv3_bn_.train_var,self.conv4_bn_.train_mean,\
            self.conv4_bn_.train_var,self.conv5_bn_.train_mean, self.conv5_bn_.train_var,self.conv1_bn.train_mean,self.conv1_bn.train_var,self.conv2_bn.train_mean,\
            self.conv2_bn.train_var,self.conv6_bn.train_mean,self.conv6_bn.train_var,self.conv7_bn.train_mean, self.conv7_bn.train_var,self.conv8_bn.train_mean, \
            self.conv8_bn.train_var,self.conv9_bn.train_mean, self.conv9_bn.train_var], feed_dict={ self.x: x,self.x_: x_, self.y: y,self.is_training: is_training})
        
        
def main():
    #training data and test data prepare
    an = ActorNet()
    model_path='/home/wzl/design/pre_train_model/'
    ...
    for epoch in range(training_epochs):
        for idx in range(int(ntrain/nbatch)):
            ...
            cost = an.train_actor(data,data_,templabel)
            index = epoch*750 + idx 
