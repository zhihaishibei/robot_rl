...
class CriticNet:
    ...
    def __init__(self, num_states, num_actions):
        ...
        with self.g.as_default():
            ...
            self.H1_c_bn = batch_norm(self.H1_t, self.is_training, self.sess)

            self.H2_t = tf.matmul(self.H1_c_bn, self.W2_c) + \
                tf.matmul(self.critic_action_in, self.W2_action_c)
            self.H2_c_bn = batch_norm(self.H2_t, self.is_training, self.sess)

            self.critic_q_model = tf.matmul(self.H2_c_bn, self.W3_c) + self.B3_c
           ...
           # Target Critic Q Network:
            ...
            self.t_H1_c_bn = batch_norm(self.t_H1_t, self.is_training, self.sess)

            self.t_H2_t = tf.matmul(self.t_H1_c_bn, self.t_W2_c) + \
                tf.matmul(self.t_critic_action_in, self.t_W2_action_c)
            self.t_H2_c_bn = batch_norm(self.t_H2_t, self.is_training, self.sess)

            self.t_critic_q_model = tf.matmul(self.t_H2_c_bn, self.t_W3_c) + self.t_B3_c
